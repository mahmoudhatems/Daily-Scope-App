     ...
     return await Future.delayed(const Duration(seconds: 2), () async {
      ...
      for (var article in articles) {
        ArticleModel articleModel = ArticleModel(
          ...
          title:
              article["title"] ?? 'No Title', // Provide a default title if null
          description: article["description"] ??
              'No Description', // Provide a default description if null
          subTitle: '',
          source: Source(
            id: article['source']['id'] ??
                'unknown', // Provide a default id if null
            name: article['source']['name'] ??
                'Unknown', // Provide a default name if null
          ),
        );
        articleList
            .add(articleModel); // Don't forget to add the article to the list
      }
      return articleList;
    });
  }
